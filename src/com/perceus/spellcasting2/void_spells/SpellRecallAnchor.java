...
public class SpellRecallAnchor extends BaseSpellCapsule
{
	...
	public boolean cast(PlayerInteractEvent event)
	{
		...
		{
			PlayerDataMana.getPlayerData(event.getPlayer()).setCurrentMana(PlayerDataMana.getPlayerData(event.getPlayer()).getCurrentMana() - 350);
			...
			playerStoredLocations.put(event.getPlayer(), event.getPlayer().getLocation());
		...
		if (event.getAction().equals(Action.RIGHT_CLICK_AIR)) 
		{
			...
			if (!playerStoredLocations.containsKey(event.getPlayer())) 
			{
			...
			SpellParticles.drawDisc(event.getPlayer().getLocation(), 2, 2, 60, Particle.PORTAL, null);
			event.getPlayer().playSound(event.getPlayer().getLocation(), Sound.ENTITY_SHULKER_SHOOT, SoundCategory.MASTER, 1, 1);
			event.getPlayer().teleport(playerStoredLocations.get(event.getPlayer()));
			new BukkitRunnable()
			{
				 @Override
				  public void run()
