...
public class BoonOfTheSeaGod extends BaseSpellCapsule
{

	public BoonOfTheSeaGod()
	{
		super(Material.NAME_TAG, "§r§aRelic§r§f: Boon of the Sea God", "BoonOfTheSeaGod", 0, true, "§r§fElement: §aPri§bmor§edi§6al§f.",
				"§r§fSpell Type: §a§6Buff§r§f.",
				"§r§fA talisman capable of permanently granting", 
				"§r§fConduit Power and quick swimming", "§r§fwhile the boon is in the offhand.",
				"§r§fRight-Click to activate the item.", "§r§fIf the caster runs out of mana,",
				"§r§fthe boon's power will be broken", "§r§fand readded to the inventory.",
				"§r§fMana cost: 30 §9mana§f/second.");
	}
	...
	private static void boonRunnable(PlayerInteractEvent event, ItemStack offHand) 
	{
		...
		new BukkitRunnable() 
		{
			...
			{        
				...
				if (!ItemUtils.readFromNamespacedKey(event.getPlayer().getInventory().getItem(EquipmentSlot.OFF_HAND), "spellname").equals("BoonOfTheSeaGod"))
				{
					this.cancel();
					event.getPlayer().playSound(event.getPlayer().getLocation(), Sound.BLOCK_CONDUIT_DEACTIVATE, SoundCategory.MASTER, 1, 1);
					PrintUtils.sendMessage(event.getPlayer(),"The boon has been unequipped.");
				...
				event.getPlayer().addPotionEffect(new PotionEffect(PotionEffectType.CONDUIT_POWER, 40, 0, true));
				event.getPlayer().addPotionEffect(new PotionEffect(PotionEffectType.DOLPHINS_GRACE, 40, 0, true));
