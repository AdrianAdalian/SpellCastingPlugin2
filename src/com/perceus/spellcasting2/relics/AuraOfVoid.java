...
public class AuraOfVoid extends BaseSpellCapsule
{

	public AuraOfVoid()
	{
		super(Material.NAME_TAG, "§r§fAura of §3§lVOID", "AuraOfVoid", 0, true, "§r§fElement: §aPri§bmor§edi§6al§f.",
				"§r§fSpell Type: §3Aura§f.",
				"§r§fA talisman that emits an aura of §3§lVOID§f energy.",
				"§r§fRight-Click to activate the item.", 
				"§r§fCancels gravity for those within 10 meters.",
				"§r§fLasts for 3 seconds outside of range.",
	...
	private static void boonRunnable(PlayerInteractEvent event, ItemStack offHand) 
	{
		...
		new BukkitRunnable() 
		{
			...
			{        
				...
				if (!ItemUtils.readFromNamespacedKey(event.getPlayer().getInventory().getItem(EquipmentSlot.OFF_HAND), "spellname").equals("AuraOfVoid"))
				{
					this.cancel();
					event.getPlayer().playSound(event.getPlayer().getLocation(), Sound.BLOCK_BEACON_DEACTIVATE, SoundCategory.MASTER, 1, 1);
					PrintUtils.sendMessage(event.getPlayer(),"The talisman has been unequipped.");
				...
				for (Entity target : event.getPlayer().getNearbyEntities(10, 10, 10)) 
				{
					...
					((LivingEntity) target).addPotionEffect(new PotionEffect(PotionEffectType.LEVITATION, 60, 1, true));
				}
			}
