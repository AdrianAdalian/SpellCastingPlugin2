...
public class BoonOfAntiGravity extends BaseSpellCapsule
{

	public BoonOfAntiGravity()
	{
		super(Material.NAME_TAG, "§r§aRelic§r§f: Boon of Anti Gravity", "BoonOfAntiGravity", 0, true, "§r§fElement: §aPri§bmor§edi§6al§f.",
				"§r§fSpell Type: §a§6Buff§r§f.",
				"§r§fA talisman capable of permanently granting", 
				"§r§fJump Boost and Slow Falling", "§r§fwhile the boon is in the offhand.",
	...
	private static void boonRunnable(PlayerInteractEvent event, ItemStack offHand) 
	{
		...
		new BukkitRunnable() 
		{
			...
			{        
				...
				if (!ItemUtils.readFromNamespacedKey(event.getPlayer().getInventory().getItem(EquipmentSlot.OFF_HAND), "spellname").equals("BoonOfAntiGravity"))
				{
					this.cancel();
					event.getPlayer().playSound(event.getPlayer().getLocation(), Sound.BLOCK_BEACON_DEACTIVATE, SoundCategory.MASTER, 1, 1);
					PrintUtils.sendMessage(event.getPlayer(),"The boon has been unequipped.");
				...
				event.getPlayer().addPotionEffect(new PotionEffect(PotionEffectType.JUMP, 40, 2, true));
