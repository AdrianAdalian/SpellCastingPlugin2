...
public class BoonOfWaterbreathing extends BaseSpellCapsule
{

	public BoonOfWaterbreathing()
	{
		super(Material.NAME_TAG, "§r§aRelic§r§f: Boon of Water Breathing", "BoonOfWaterBreathing", 0, true, "§r§fElement: §aPri§bmor§edi§6al§f.",
				"§r§fSpell Type: §a§6Buff§r§f.",
				"§r§fA talisman capable of permanently granting", 
				"§r§fWater Breathing while the boon is in the offhand.",
				...
				"§r§fMana cost: 10 §9mana§f/second.");
	}
	...
	private static void boonRunnable(PlayerInteractEvent event, ItemStack offHand) 
	{
		...
		new BukkitRunnable() 
		{
			...
			{        
				...
				if (!ItemUtils.readFromNamespacedKey(event.getPlayer().getInventory().getItem(EquipmentSlot.OFF_HAND), "spellname").equals("BoonOfWaterBreathing"))
				{
					this.cancel();
					event.getPlayer().playSound(event.getPlayer().getLocation(), Sound.BLOCK_CONDUIT_DEACTIVATE, SoundCategory.MASTER, 1, 1);
					PrintUtils.sendMessage(event.getPlayer(),"The boon has been unequipped.");
