...
public class BoonOfDefense extends BaseSpellCapsule
{

	public BoonOfDefense()
	{
		super(Material.NAME_TAG, "§r§aRelic§r§f: Boon of Defense", "BoonOfDefense", 0, true, "§r§fElement: §aPri§bmor§edi§6al§f.",
				"§r§fSpell Type: §a§6Buff§r§f.",
				"§r§fA talisman capable of permanently granting", 
				"§r§fResistance while the boon is in the offhand.",
	...
	private static void boonRunnable(PlayerInteractEvent event, ItemStack offHand) 
	{
		...
		new BukkitRunnable() 
		{
			...
			{        
				...
				if (!ItemUtils.readFromNamespacedKey(event.getPlayer().getInventory().getItem(EquipmentSlot.OFF_HAND), "spellname").equals("BoonOfDefense"))
				{
					this.cancel();
					event.getPlayer().playSound(event.getPlayer().getLocation(), Sound.BLOCK_BEACON_DEACTIVATE, SoundCategory.MASTER, 1, 1);
					PrintUtils.sendMessage(event.getPlayer(),"The boon has been unequipped.");
