...
public class BoonOfTheAngels extends BaseSpellCapsule
{

	public BoonOfTheAngels()
	{
		super(Material.NAME_TAG, "§r§aRelic§r§f: Boon of the Angels", "BoonOfTheAngels", 0, true, "§r§fElement: §aPri§bmor§edi§6al§f.",
				"§r§fSpell Type: §a§6Buff§r§f.",
				"§r§fA talisman capable of permanently granting", 
				"§r§fmax health and infinite flight", 
				"§r§fwhile the boon is in the offhand.",
	...
	private static void boonRunnable(PlayerInteractEvent event, ItemStack offHand) 
	{
		...
		new BukkitRunnable() 
		{
			...
			{        
				...
				if (!ItemUtils.readFromNamespacedKey(event.getPlayer().getInventory().getItem(EquipmentSlot.OFF_HAND), "spellname").equals("BoonOfTheAngels"))
				{
					this.cancel();
					event.getPlayer().playSound(event.getPlayer().getLocation(), Sound.BLOCK_BEACON_DEACTIVATE, SoundCategory.MASTER, 1, 1);
					event.getPlayer().setAllowFlight(false);
				...
				event.getPlayer().addPotionEffect(new PotionEffect(PotionEffectType.HEALTH_BOOST, 40, 4, true));
			}
