...
public class SpellTidalForce extends BaseSpellCapsule
{
		...
		if (event.getPlayer().isSwimming() == true)
		{
			...
			new BukkitRunnable()
			{
				 @Override
				  public void run()
				  {
						event.getPlayer().playSound(event.getPlayer().getLocation(), Sound.ITEM_TRIDENT_RIPTIDE_2, SoundCategory.MASTER, 1, 1);
				  }
